...
Goal: Measure how much reranking improves quality, and what it costs in latency.
...
class Experiment2BestRerank:
    """Experiment 2: Best Voyage model with reranking for quality improvement."""
    ...
    def __init__(self, config_path: str = "config/experiment_config.yaml"):
        ...
        self.exp_config = self.config["experiments"]["experiment_2_best_rerank"]
        ...
        self.logger = ExperimentLogger("experiment_2_best_rerank", self.config)
        
        print(f"✓ Initialized {self.exp_config['name']}")
        print(f"  Embedding Model: {self.model_config['model_name']}")
...
def main():
    ...
    try:
        experiment = Experiment2BestRerank()
